...
src-tauri/src/
  ...
  orchestration/    agent groups: registry, guardrails, MCP server, audit. Compact-survival is
    ...
    compaction diluting it; both Claude's `PreCompact`/`SessionStart` hooks AND Copilot's own
    ...
    `SessionStart(compact)` re-grounding signal Copilot's docs confirm has no equivalent;
    banner/token-drop/marker inference remains the fallback tier for hook-less setups. See
    doc/design/orchestration.md's Compact-nudge section.
