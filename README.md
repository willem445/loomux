...
3. **Agent orchestration, native** — a planning agent delegates GitHub issues
   ...
   discover by accident. Prompts that pile up behind a blocked pane are
   delivered as ONE itemized prompt when it clears — oldest first, each
   still labelled with who sent it and how long it waited — rather than
   costing the agent a turn apiece, and an exit the orchestrator itself
   asked for (a kill, an idle-timeout) goes to the audit log instead of
   interrupting it, since the roster already shows who is alive. Hard-won lessons persist
