...
4. **Write tests that test intent — then watch them fail.** A test must fail if the feature
   ...
   **Red before green, evidenced.** Commit the tests FIRST, as their own commit(s) on the
   branch, and redden that tree before the feature commit goes on top — never `git stash` the
   implementation to strip it (the stash stack is shared across every worktree of this repo,
   #299; commit-first is #493). Rust red comes from CI, since local `cargo` is banned;
   frontend red is `node --test` on that same committed tree, locally, in a second. Every new
   test that touches only *pre-existing* public API belongs in that tests-only commit: the
   test is "does it compile without my change", not "is it about my feature". A pin that
   shipped alongside the feature anyway is evidenced after the fact — one commit removing
   exactly the thing under test, the run naming which test noticed, then restore (#745).
   Confirm they fail for the *expected* reason — not on a
