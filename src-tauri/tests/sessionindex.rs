...
fn seam() -> Seam {
    ...
    // #2515 C2: the fifth source, bound for the reason above — and codex is the
    // one most likely to be non-empty on a developer's machine, since `~/.codex`
    // is where the OpenAI desktop app writes too. Its rows would count against
    // LIST_LIMIT here.
