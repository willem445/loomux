...
//!
//!   - `generate_handler_matches_app_commands` / `app_commands_len_is_135`:
...
//!   - `main_has_all_135_and_zero_permission_denies_dangerous_spread`: builds
...
//!     invokes all 135 commands against the `main` window label, and invokes
...
//! `main_has_all_135_and_zero_permission_denies_dangerous_spread` fail with
...
#[test]
fn app_commands_len_is_135() {
    assert_eq!(
        loomux_lib::command_manifest::APP_COMMANDS.len(),
        135,
        "APP_COMMANDS drifted from the expected count of 135 (120 per the #363 plan's audited \
         ...
         model-knob capability query added in #687) — if this is an intentional \
         addition/removal, update this tripwire's count too"
    );
}

#[test]
fn main_has_all_135_and_zero_permission_denies_dangerous_spread() {
