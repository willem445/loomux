...
//!     than trusting a hand count. This feature's own seven commands are
//!     #360 Slice B's `list_plugins`/`install_plugin`, Slice C's three broker
//!     commands (`plugin_open_window`, `plugin_broker_request`,
//!     `plugin_broker_open_channel`), Slice D's `plugin_close_window` (the
//!     child-webview embedding's explicit-close command — see
//!     `pluginbroker.rs`'s module doc comment on why a child webview has no
//!     `WindowEvent::Destroyed` to hook cleanup onto instead), and #391's
