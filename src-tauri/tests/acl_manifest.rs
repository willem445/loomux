...
//!
//!   - `generate_handler_matches_app_commands` / `app_commands_len_is_129`:
...
//!   - `main_has_all_129_and_zero_permission_denies_dangerous_spread`: builds
...
//!     invokes all 129 commands against the `main` window label, and invokes
...
//! `main_has_all_129_and_zero_permission_denies_dangerous_spread` fail with
...
#[test]
fn app_commands_len_is_129() {
    assert_eq!(
        loomux_lib::command_manifest::APP_COMMANDS.len(),
        129,
        "APP_COMMANDS drifted from the expected count of 129 (120 per the #363 plan's audited \
...
#[test]
fn main_has_all_129_and_zero_permission_denies_dangerous_spread() {
