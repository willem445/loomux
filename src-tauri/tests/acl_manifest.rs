...
//!
//!   - `generate_handler_matches_app_commands` / `app_commands_len_is_136`:
...
//!   - `main_has_all_136_and_zero_permission_denies_dangerous_spread`: builds
...
//!     invokes all 136 commands against the `main` window label, and invokes
...
//! `main_has_all_136_and_zero_permission_denies_dangerous_spread` fail with
...
stub_commands!(
    ...
    agent_autopilot_flags, agent_cli_knobs, create_orchestration, bind_agent, orch_agent_renamed, orch_session_roles,
...
#[test]
fn app_commands_len_is_136() {
    assert_eq!(
        loomux_lib::command_manifest::APP_COMMANDS.len(),
        136,
        "APP_COMMANDS drifted from the expected count of 136 (120 per the #363 plan's audited \
         ...
         model-knob capability query added in #687, +1 for orch_set_full_autonomy — the \
         full-autonomy toggle added in #778) — if this is an intentional \
         addition/removal, update this tripwire's count too"
    );
}

#[test]
fn main_has_all_136_and_zero_permission_denies_dangerous_spread() {
