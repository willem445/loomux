...
- **#1175, board WIP limits** — `orchestrator.md` only, one bullet in **The task board**
  ...
  now declare per-status caps (`board.wip` in `.orrerix/workflow.yml`); `list_tasks` carries
  ...
  read it: under `enforce: false` orrerix notifies and the write still lands, so an
