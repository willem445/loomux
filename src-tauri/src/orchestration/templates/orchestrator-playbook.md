...
forward once, then revert. Do not resurrect proactive rebasing as a second safety net beside
it. The merge queue is unaffected: its speculative batch remains the mergeability probe for
sub-PRs onto an integration branch — the speculative merge *is* the probe, and only a real
conflict kicks back.
