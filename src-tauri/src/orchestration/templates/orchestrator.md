...
3. **Disposition every finding** (INVARIANT 3). A reviewer may approve *and still leave findings
   ...
   *for*, and a PR that merges with them dropped is procedurally green and materially worse. So
   ...
   - **Some "non-blocking" findings are blocking, and that call is yours.** A finding that
     ...
     *itself* carries a finding the reviewer labelled **blocking** is a contradiction — that is a
     `fail`, not a `pass` with a note. Don't merge on it; tell the reviewer its verdict didn't
     match its own findings.)
...
- **The first real conflict is where you stop.** Route it to the **owning worker** (resume its
  ...
  understand.
- **A rebase is a push**, so pay its price knowingly: CI re-runs, and every reviewer verdict goes
  stale (the reviewer re-reviews the new head, or the gate refuses the merge). That cost is the
  argument for paying it *early and in the quiet* — the alternative is paying it on the PR you
  were about to land.
