        ...
        tools.extend([
            ...
            tool("upsert_task",
                ...
                json!({
                    ...
                    "parent": { "type": "string", "description": "Task id this one sits INSIDE — its epic/feature/container. Omit = untouched, EMPTY STRING = clear (promote to top level), same rule as pr. Must name a live task on this board; a self-parent, a chain that would loop (reparenting under your own descendant), a chain deeper than 4 levels counting whatever this row already carries below it, and a parent that also appears in this row's deps/related are all rejected outright, with the error naming the path. Containment is NOT ordering — but readiness climbs it: a row reads `ready: false` while ANY container above it still has unmet `deps` of its own, since you cannot start a slice whose feature is itself waiting. An ancestor's STATUS is never read, so a child of a container merely marked `blocked` IS still startable. Express ordering with `deps`; use `parent` for where the work belongs." },
