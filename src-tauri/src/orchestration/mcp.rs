        ...
        tools.extend([
            tool("spawn_agent",
                ...
                json!({
                    ...
                    "profile": { "type": "string", "description": "Custom agent profile from <repo>/.loomux/agents/<name>.md — sets persona instructions, model, extra MCP tools. Unknown names error with the available list." },
...
fn call_tool(reg: &OrchRegistry, caller: &Caller, name: &str, args: &Value) -> Result<String, String> {
    match name {
        ...
        "spawn_agent" => {
            ...
            let a = reg.spawn_agent_ex(&caller.group, kind, name, task, worktree, branch, profile, resume, cwd)?;
            Ok(format!(
