...
fn arg_str_array(args: &Value, key: &str) -> Result<Option<Vec<String>>, String> {
    match args.get(key) {
        ...
        Some(Value::Array(items)) => items
            ...
            .map(Some),
        // STUB (red-evidence commit): a wrongly-typed arg is silently ignored.
        Some(_) => Ok(None),
    }
}
...
fn arg_bool(args: &Value, key: &str) -> Result<bool, String> {
    match args.get(key) {
        ...
        // STUB (red-evidence commit): a wrongly-typed arg silently reads as false.
        Some(_) => Ok(false),
    }
}
