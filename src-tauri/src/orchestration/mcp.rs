        ...
        tools.extend([
            tool("spawn_agent",
                ...
                json!({
                    ...
                    "profile": { "type": "string", "description": "Custom agent profile from <repo>/.github/agents/<name>.md (GitHub Copilot agents.md convention): sets persona instructions, model, and — when the group trusts this repo — extra MCP tools. mode:append layers on the built-in role contract; mode:replace swaps the persona/policy body while loomux still injects its mechanics core (MCP tools, board, report(), git/PR flow). A profile's role/kind can retarget the spawn (e.g. a reviewer persona). Unknown names error with the available list. Omit to use the role's manual/auto assignment (auto uses append-mode files only; replace must be named explicitly here)." },
