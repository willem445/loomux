...
- **Merge gate** — when an item reaches `pr` or `human-testing` (the point where
  ...
  has its own merge gate armed and unsatisfied, Approve is relabeled up front
  (e.g. "Approve (won't merge — gate needs rev-orch/rev-ui/rev-tests)") so you
  know before you click, and the tooltip names your options: run the missing
  reviewers, toggle the workflow off, or merge via the GitHub UI directly.
