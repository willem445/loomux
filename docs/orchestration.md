...
- **Merge gate** — when an item reaches `pr` or `human-testing` (the point where
  ...
  orchestrator pane, exactly as if you'd typed it. Note that Approve is *your*
  merge gate, not the repo's — if a [custom workflow](#custom-agent-workflows)
  has its own merge gate armed, Approve is relabeled up front whenever the
  item carries a PR (e.g. "Approve (won't merge — gate needs
  rev-orch/rev-ui/rev-tests)") so you know before you click, and the tooltip
  names your options: run the missing reviewers, toggle the workflow off, or
  merge via the GitHub UI directly. The relabel keys on "does this item carry
  a PR at all," not that PR's target branch — a stacked sub-PR merging into an
  integration branch (not the gated default branch) gets the same warning as
  one merging straight to `main`. Deliberately conservative: the board only
  knows a task's PR number, not its base ref, and narrowing that would mean
  either a live `gh pr view` per row or a new field on the task record — worth
  doing if this proves noisy in practice, not assumed cheap today.
