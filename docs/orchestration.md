...
- **Delivery queue.** If a pane is busy — an interactive question on screen, or a
  ...
  *forward*: if the hold is the interactive-question one, and after 15 minutes the
  pane's own screen is plainly showing an empty input box with no menu on it,
  loomux stops believing its own question detection and delivers anyway (you'll
  have had the "stuck behind a question" badge for five minutes by then). A hold
  on your own typed input is never overridden, at any age.
  You'll never see "held … re-send" — if a prompt is safely
