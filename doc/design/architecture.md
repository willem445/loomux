...
src/
  ...
  paneautosize.ts   pure weight math for the Autosize gesture (#936): every node weighted by the number of PANES under it, which is what makes the leaves equal — weighting each split's children equally does not, since a pane's size is the product of its share at every level. Takes the tree's shape only (no panes, no elements, no measurement) and returns a WeightNode tree for applyLayoutWeights. On demand only: splits keep paying out of the pane you split, and nothing re-levels a layout by itself. DOM-free, unit-tested. See doc/design/pane-autosize.md
