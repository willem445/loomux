...
src-tauri/src/
  ...
  command_manifest.rs  single source of truth for the ACL manifest's 139 app-command names (#363) — shared by build.rs (include!, feeds the app manifest) and lib.rs (feeds tests/acl_manifest.rs, the coherence guard). See doc/design/acl-manifest.md
  lib.rs            Tauri wiring
src-tauri/
