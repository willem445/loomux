...
src-tauri/src/
  ...
  pluginbroker.rs   the plugin TRUST CORE (#360 Slice C): the only path a plugin's webview has into loomux. A per-session broker whose vocabulary is a CLOSED capability enum — no shell, no PTY, no git/gh, no orchestration or merge-grant command is expressible in it at all — with the calling plugin derived from the webview's own label, never from a caller-supplied id. Also `plugin_open_window`/`plugin_close_window`, which are granted to `main` only and never to a plugin webview. See doc/design/pane-plugins.md's Isolation section
  pluginregion.rs   native bounds + occlusion for an embedded plugin child webview (#391, folded into #380): `plugin_set_frame` atomically sets the webview's bounds and clips its own HWND (`SetWindowRgn`/`CombineRgn(RGN_DIFF)`) to exclude whatever DOM overlay rects currently cover its pane. `SetWindowRgn` removes a region from hit-testing as well as painting, so a punched hole passes clicks through to main's DOM — this is what closes both the paint and the pointer half of #391
  plugingrants.rs   the pane-plugin capability GRANT store (#377): one persisted human decision per plugin id in `<app-data>/loomux/plugins/grants.json` (uistate's atomic-write + quarantine discipline), plus the pure subset rule `pluginbroker::validate_open_request` gates every plugin open on — no record, a corrupt store or a WIDENED manifest all fail closed. `plugin_approve_capabilities` is its one writer and is granted to main's webview only, never to a plugin (a plugin that could approve itself is the self-approval CLAUDE.md constraint 9 forbids). See doc/design/pane-plugins.md's "Grant, in v1"
  command_manifest.rs  single source of truth for the ACL manifest's 140 app-command names (#363) — shared by build.rs (include!, feeds the app manifest) and lib.rs (feeds tests/acl_manifest.rs, the coherence guard). See doc/design/acl-manifest.md
  lib.rs            Tauri wiring
src-tauri/
...
src/
  ...
  pluginhost.ts     typed bridge to plugins.rs (#360 Slice B, per-feature wrapper like fileapi.ts): list installed plugins, install one from a local folder; record a human's capability approval (#377)
  pluginconsent.ts  pure text/ordering for the plugin capability-approval prompt (#377, DOM-free, unit-tested): one plain-language line per declared capability, `fs.read`/`metrics.system` flagged and sorted first. Decides only what the human READS — the gate itself is plugingrants.rs + pluginbroker.rs
