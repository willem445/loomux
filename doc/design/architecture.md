...
src-tauri/src/
  ...
  command_manifest.rs  single source of truth for the ACL manifest's 146 app-command names (#363) — shared by build.rs (include!, feeds the app manifest) and lib.rs (feeds tests/acl_manifest.rs, the coherence guard). Don't hand-count it: `app_commands_len_is_146` pins the total and carries the per-delta provenance, so that test is what this number tracks. See doc/design/acl-manifest.md
  lib.rs            Tauri wiring
src-tauri/
