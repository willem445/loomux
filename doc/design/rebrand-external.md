...
| The bundle identifier `dev.loomux.app` | It keys the WebView2 user-data folder and the macOS bundle ID. Moving it orphans every user's webview profile, and no one outside the repo ever sees it. **Reversed in #1562**, which flips it and moves the profile once — see [rebrand-bundle.md](rebrand-bundle.md). |
