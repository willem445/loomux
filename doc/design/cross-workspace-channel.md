...
| codex, hermes, ante | delivery-only | no spawn-flag seam today — tracked in [#288](https://github.com/willem445/orrerix/issues/288). Ante's MCP config is file-based only (`~/.ante/settings.json`, no CLI flag) — see #292. Ante also runs on macOS/Linux hosts only (no Windows binary upstream). |
