...
- **Correcting a false claim is a multi-surface edit.** A design rationale here
  ...
  **That sweep's blind spot is the LINE, and its failure shape is a NON-ZERO
  receipt.** Prose and rustdoc here are hand-wrapped, so a line-oriented `grep` for
  a multi-word claim cannot match an instance the wrap split, and the same-shape
  positive control still passes because it matched the UNWRAPPED copies. Sweep the
  shortest stable fragment and cross-check a wrap-insensitive pass (join lines,
  strip `///`/`//!`, squeeze spaces) — the two totals must agree. Signature: a
  healthy non-zero receipt that a later re-sweep beats — #1346 went 1 → 2, #1408
  3 → 4, and #1667 2 → 4 (`"no timeout, no try-lock"` over three pre-fix blobs
  misses the two where the phrase straddles the wrap). Recipe:
  `.claude/skills/ci-validate/SKILL.md`.
