...
impl TickSupervisor {
    ...
    pub fn run_for<T>(&mut self, body: impl FnOnce() -> T) -> Option<T> {
        ...
        // NEUTERED (scratch round 1, neuter B): the catch is removed, so a
        // panicking body unwinds out of the supervisor and takes the loop
        // thread with it exactly as the bare loops did before #1702.
        let v = body();
