...
pub fn parse_workflow(text: &str) -> Result<Workflow, Vec<String>> {
    ...
    // Review-driver policy (#1778 §5.3). [scratch round 5] the schema surface
    // is intact (the block parses, the manifest/keys/facts agree) but the
    // PARSE WIRING is withheld: every declared value is discarded and the
    // policy resolves to the default. This is the neuter that reaches the
    // workflow binary's INNER value assertions, not just its unwraps.
