...
struct RawBlock {
    ...
    // NEUTERED FOR THE RED ROUND: the ssh-shaped keys become accepted fields.
    // `skip_serializing` keeps them out of `workflow_schema_keys`, so the #880
    // manifest pin stays green and cargo reaches the engine's own tests.
